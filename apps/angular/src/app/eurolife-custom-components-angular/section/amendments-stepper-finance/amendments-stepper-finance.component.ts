...
@Component({
  selector: "app-amendments-stepper-finance",
  templateUrl: "./amendments-stepper-finance.component.html",
  styleUrls: ["./amendments-stepper-finance.component.scss"]
})
export class AmendmentsStepperFinanceComponent extends SectionComponentImplementation {
  stepperState: any = {
  ...
  manageSteps(step: any) {
    if(step==0)
    {
      ...
      this.stepperState.stepsState[step].state="done";
      this.stepperState.stepsState[step+1].state="inactive";
    }
    if(step==1)
    {
      this.stepperState.step = step;
