...
@Component({
  selector: 'app-funds-lifecycle-product',
  templateUrl: './funds-lifecycle-product.component.html',
  styleUrls: ['./funds-lifecycle-product.component.scss']
})


export class FundsLifecycleProductComponent extends SectionComponentImplementation {
  ...
  ngOnInit() {
    ...
    if(this.parent.section.recipe.type=="RegularBasket")
    {
      ...
      this.totalValueNumber=this.iceModel.elements["policies.details.lifecycle.RegularAmount"].getValue().values[0].value;
    ...
    {
      this.totalValueNumber=this.iceModel.elements["policies.details.lifecycle.ExtraAmount"].getValue().values[0].value;
      if(this.totalValueNumber>0)
      this.showContent=true;
    }
    ...
    if (value.element.getValue().values[0].value==true && this.counter==false)
      {
        ...
        this.basket="Επενδυτική Στρατηγική-"+this.iceModel.elements["policies.details.lifecycle.BasketDescription"].getValue().values[0].value;
        this.addItems();
...
//  this.showContent=false;
  ...
  private addItems(): any {
    ...
    if(this.parent.section.recipe.type=="RegularBasket")
    {
      ...
      this.totalValue=new Intl.NumberFormat('el-GR', { style: 'decimal',maximumFractionDigits:2,minimumFractionDigits:2 }).format(this.iceModel.elements["policies.details.lifecycle.RegularAmount"].getValue().values[0].value);
      this.totalSurrenderValue=new Intl.NumberFormat('el-GR', { style: 'decimal',maximumFractionDigits:2,minimumFractionDigits:2 }).format(this.iceModel.elements["policies.details.lifecycle.RegularSurrenderValue"].getValue().values[0].value);
    }

    if(this.parent.section.recipe.type=="ExtraBasket")
    {
      this.totalValue=new Intl.NumberFormat('el-GR', { style: 'decimal',maximumFractionDigits:2,minimumFractionDigits:2 }).format(this.iceModel.elements["policies.details.lifecycle.ExtraAmount"].getValue().values[0].value);
      this.totalSurrenderValue=new Intl.NumberFormat('el-GR', { style: 'decimal',maximumFractionDigits:2,minimumFractionDigits:2 }).format(this.iceModel.elements["policies.details.lifecycle.ExtraSurrenderValue"].getValue().values[0].value);
