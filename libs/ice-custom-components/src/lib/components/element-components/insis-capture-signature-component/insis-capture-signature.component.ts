import { Component, HostListener, Input, ViewChild, OnInit, ElementRef, Inject, AfterViewInit } from '@angular/core';
import { IceTextInputComponent, MaterialElementComponentImplementation, IceContextService } from '@impeo/ng-ice';
...
export class InsisCaptureSignatureComponent extends MaterialElementComponentImplementation implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.getRecipeParam('qrCodeUrlElement'))
      this.qrCodeUrlElement = this.context.iceModel.elements[this.getRecipeParam('qrCodeUrlElement')] as ItemElement;
    this.qrCodeResource = this.resource.getEntry(this.getRecipeParam('qrCodeResource'));
    if (this.getRecipeParam('hideQrCodeOnMobile')) this.hideQrCodeOnMobile = this.getRecipeParam('hideQrCodeOnMobile');
    ...
    this.element.$dataModelValueChange
