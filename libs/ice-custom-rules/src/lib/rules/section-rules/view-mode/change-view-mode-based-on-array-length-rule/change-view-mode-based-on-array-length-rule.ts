import { SectionViewModeRule, SectionViewMode, IceConsole, ArrayElement } from '@impeo/ice-core';
...
export class ChangeViewModeBasedOnArrayLengthRule extends SectionViewModeRule {
  getViewMode(actionContext?: any): SectionViewMode {
    ...
    if (array.type !== 'array') {
      IceConsole.warn(
        `${ChangeViewModeBasedOnArrayLengthRule.name} specified element is not of type array`
      );
    ...
    if (!condition || !Object.values(Conditions).includes(condition)) {
      IceConsole.warn(
        `${ChangeViewModeBasedOnArrayLengthRule.name} condition param is not set or is not a correct condition value (lesser, equals, greater).`
      );
      return SectionViewMode.DEFAULT;
    }
    ...
    if (conditionValue === NaN) {
      IceConsole.warn(
        `${ChangeViewModeBasedOnArrayLengthRule.name} conditionValue is not set or is not an integer.`
      );
      return SectionViewMode.DEFAULT;
    }
    ...
    if (!viewMode || viewMode.trim() === '' || !Object.values(SectionViewMode).includes(viewMode)) {
      IceConsole.warn(
        `${ChangeViewModeBasedOnArrayLengthRule.name} viewmode param is not set or is invalid.`
      );
      return SectionViewMode.DEFAULT;
    }
    ...
    if (elseViewMode && !Object.values(SectionViewMode).includes(elseViewMode)) {
      IceConsole.warn(
        `${ChangeViewModeBasedOnArrayLengthRule.name} elseViewMode is invalid viewmode. Fallsback on default view mode if length is different`
      );
