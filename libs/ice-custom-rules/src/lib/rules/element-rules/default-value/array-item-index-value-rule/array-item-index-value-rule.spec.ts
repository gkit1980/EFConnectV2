...
import { ArrayItemIndexValueRule } from './array-item-index-value-rule';

describe(ArrayItemIndexValueRule.name, () => {
  let iceTesting: IceTesting;
  beforeEach(
    () =>
      (iceTesting = IceTesting.initializeTestingRuntime().registerRule(
        RuleHolder.IceElement,
        ArrayItemIndexValueRule
      ))
  );
  ...
  const createIceContext = async (ruleRecipe: any) =>
    await iceTesting
      ...
      .element(elementName, 'text', element =>
        element.defaultValueRule(ArrayItemIndexValueRule.name, ruleRecipe)
      )
      .build();
  ...
  it('sets elements value to the index it has in a array', async () => {
    ...
    const rule = <ArrayItemIndexValueRule>element.defaultValueRule;
