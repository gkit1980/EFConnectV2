...
//
//
export class ArrayItemBasedValuesRule extends ValuesRule {
  ...
  protected getItems(index: number[] | null): any[] {
    const items = map(this.valueElement.getValue().values, value => {
      return { value: value.value, label: this.labelElement.getValue().forIndex(value.index) };
    });
    const emptyItemResourceKey = this.element.name + '.empty';
    items.push({
      value: null,
      label: this.resource.resolve(emptyItemResourceKey, 'Use a different bank account')
    });
    return items;
  }
