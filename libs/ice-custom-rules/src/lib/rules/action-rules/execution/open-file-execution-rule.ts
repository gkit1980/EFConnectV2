...
export class OpenFileExecutionRule extends ExecutionRule {
	...
	private base64StringToBlob(data: string): Blob {
		...
		//
		if(this.context.iceModel.elements[this.recipe['filetype']].getValue().forIndex(null)=="pdf")
		return new Blob([view], { type: "application/pdf" });

		if(this.context.iceModel.elements[this.recipe['filetype']].getValue().forIndex(null)=="doc")
		return new Blob([view], { type: "application/doc" });

		if(this.context.iceModel.elements[this.recipe['filetype']].getValue().forIndex(null)=="jpeg")
		return new Blob([view], { type: "application/jpeg" });

		if(this.context.iceModel.elements[this.recipe['filetype']].getValue().forIndex(null)=="jpg")
		return new Blob([view], { type: "application/jpg" });

		if(this.context.iceModel.elements[this.recipe['filetype']].getValue().forIndex(null)=="png")
		return new Blob([view], { type: "application/png" });
    ...
    const FileName = this.recipe['filename'] ? (this.recipe['filename'] as string) : 'document';
    const cur_timestamp = moment().format('DD_MM_YYYY_HH:mm:ss');
    let filename = `${FileName}_${cur_timestamp}`;
